...
def image_transform(
    ...
    *,
    ...
    if handwriting:
        ...
        starters = _build_given_mask(board, given_mask, starters_ratio, starters_count, starters_seed)
    ...
    for r in range(N):
        for c in range(N):
            ...
            while mini >= 8 and tries < 6:
                seq_img = _render_inline_candidates(generator, opts, fill_color,
    ...
    for r in range(N):
        for c in range(N):
            ...
            x0, y0, x1, y1 = cell_rect(r, c)
            text = str(v) if v <= 9 else alphabet[v-10] if 0 <= (v-10) < len(alphabet) else str(v)
            ...
            if starters[r, c]:
                ...
                if text.isdigit() and (generator is not None):
                    ...
                    if arr.shape[-1] == 4:
                        arr[:, :, 3] = np.clip(arr[:, :, 3].astype(np.float32) * 1.25, 0, 255).astype(np.uint8)
    ...
    # Outer border
    draw.rectangle([margin_px, margin_px, W - margin_px, H - margin_px],
    ...
    return sudoku_image_preprocess(img, standardize=standardize, to_tensor=True)
