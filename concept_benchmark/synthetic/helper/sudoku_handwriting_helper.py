...
class SimpleHandwrittenGenerator: 
    ...
    def generate(self, digit, size=64, color=(91,58,99), rng_seed=None):
        ...
        x_offset = random.randint(-size//4, size//4)
        y_offset = random.randint(-size//4, size//4)
        scale_factor = random.uniform(0.8, 1.2)
        blur_radius = random.choice([0, 0, 0, 0.5, 1.0])  # Most times no blur
        # Choose random font
        ...
        if random.random() > 0.7:
            result_img = self._vary_thickness(result_img)
        return result_img
    
    def _vary_thickness(self, img):
        """
