...
class SimpleHandwrittenGenerator:
    ...
    def generate(self, digit, size=64, color=(91, 58, 99), rng_seed=None):
        ...
        if not self.font_files:
            raise ValueError(
                "No fonts available. Please add TTF files to the fonts directory"
            )
        ...
        try:
            ...
            logging.getLogger(__name__).warning(
                "Error loading font %s: %s", font_file, e
            )
            # Fallback to default
...
def _build_given_mask(
    ...
    if starters_seed is None:
        ...
        starters_seed = int(
            np.sum(board * np.arange(1, board.size + 1).reshape(board.shape)) % (2**31)
        )
...
def _synthesize_prior_from_neighbors(
    ...
    *,
    ...
    for sr, sc in seeds:
        ...
        group_digits = {
            int(board[rr, cc]) for (rr, cc) in group if int(board[rr, cc]) > 0
        }
...
def _render_inline_candidates(generator, opts, color, size, seed_base=0):
    ...
    for j, d in enumerate(opts):
        ...
        if arr.shape[-1] == 4:
            arr[:, :, 3] = np.clip(
                arr[:, :, 3].astype(np.float32) * 1.8, 0, 255
            ).astype(np.uint8)
