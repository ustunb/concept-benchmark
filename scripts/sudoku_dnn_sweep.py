...
- Selective DNN baseline (image DNN with abstention)
- No-intervention conceptual safeguard baseline (same strategy, no concept fixes)
- Standard CBM DNN (no abstention, no interventions)

We support concept missingness:
...
DNN_STRATEGY_NAME = "selective_dnn_cbm"  # kept name for compatibility
NOINT_STRATEGY_NAME = "conceptual_safeguards_no_intervention"
...
# -------------------------------------------------------------------------
# Simple Image DNN: Sudoku image -> logits over labels
# -------------------------------------------------------------------------
IMG_SIZE = 96  # resize all images to 96x96


def default_image_transform() -> T.Compose:
    """
    Default transform for Sudoku images:
    - Grayscale (1 channel)
...
class SudokuImageDNN(nn.Module):
    """
    Simple CNN for Sudoku board image classification.

    Assumes input shape (N, 1, IMG_SIZE, IMG_SIZE).
    """

    def __init__(self, num_classes: int):
        super().__init__()
        self.conv1 = nn.Conv2d(1, 32, kernel_size=3, padding=1)   # -> 32 x 96 x 96
        self.conv2 = nn.Conv2d(32, 64, kernel_size=3, padding=1)  # -> 64 x 48 x 48
        self.pool = nn.MaxPool2d(2, 2)  # halves H/W each time

        # After two pools: 96 -> 48 -> 24
        conv_out_h = IMG_SIZE // 4
        conv_out_w = IMG_SIZE // 4
        conv_out_dim = 64 * conv_out_h * conv_out_w

        self.fc1 = nn.Linear(conv_out_dim, 128)
        self.fc2 = nn.Linear(128, num_classes)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = self.pool(F.relu(self.conv1(x)))  # (N,32,48,48)
        x = self.pool(F.relu(self.conv2(x)))  # (N,64,24,24)
        x = torch.flatten(x, 1)
        x = F.relu(self.fc1(x))
        x = self.fc2(x)  # logits
        return x
...
def train_cnn_cbm_from_sidecar(
    *,
    ...
    def _get_base_concepts(split, tag: str) -> np.ndarray:
        ...
        # Concepts are binary indicators in {0,1}
...
# -------------------------------------------------------------------------
# Train image DNN on dataset splits
# -------------------------------------------------------------------------
def train_image_dnn_from_dataset(
    dataset,
    epochs: int = 5,
    batch_size: int = 64,
    verbose: bool = True,
) -> Tuple[SudokuImageDNN, T.Compose]:
    """
    Train a simple image DNN on the dataset's training split, validating on val.

    - Uses dataset.X (paths) and dataset.y (labels).
    - Returns the trained model + the transform used.
    """
    split_map = {name.lower(): split for name, split in iter_splits(dataset)}
    train_split = split_map.get("training") or split_map.get("train")
    val_split = split_map.get("validation") or split_map.get("val")
    ...
    if train_split is None or val_split is None:
        raise RuntimeError(
            "[Image DNN train] Dataset must have training and validation splits."
        )

    transform = default_image_transform()

    train_X_paths = list(_raw_X_from_split(train_split))
    train_y = np.asarray(train_split.y, dtype=np.int64)

    val_X_paths = list(_raw_X_from_split(val_split))
    val_y = np.asarray(val_split.y, dtype=np.int64)

    n_classes = int(np.max(train_y)) + 1
    model = SudokuImageDNN(num_classes=n_classes).to(device)

    opt = torch.optim.Adam(model.parameters(), lr=1e-3)
    criterion = nn.CrossEntropyLoss()

    def make_batches(X_paths, y_arr, shuffle: bool = True):
        idx = np.arange(len(X_paths))
        if shuffle:
            np.random.shuffle(idx)
        for i in range(0, len(idx), batch_size):
            j = idx[i : i + batch_size]
            batch_paths = [X_paths[k] for k in j]
            batch_y = torch.from_numpy(y_arr[j]).to(device)

            imgs = []
            for p in batch_paths:
                img = Image.open(p).convert("RGB")
                imgs.append(transform(img))
            X_batch = torch.stack(imgs, dim=0).to(device)
            yield X_batch, batch_y
    ...
    for ep in range(epochs):
        ...
        train_loss = 0.0
        n_train = 0
        for X_batch, y_batch in make_batches(train_X_paths, train_y, shuffle=True):
            opt.zero_grad()
            logits = model(X_batch)
            loss = criterion(logits, y_batch)
            loss.backward()
            opt.step()
            train_loss += float(loss.item()) * y_batch.size(0)
        ...
        correct = 0
        with torch.no_grad():
            for X_batch, y_batch in make_batches(val_X_paths, val_y, shuffle=False):
                logits = model(X_batch)
                loss = criterion(logits, y_batch)
                val_loss += float(loss.item()) * y_batch.size(0)
        ...
        if verbose:
            print(
                f"[Image DNN train] Epoch {ep+1}/{epochs}: "
...
# -------------------------------------------------------------------------
# Image-based Selective DNN baseline (no interventions, but abstains)
# -------------------------------------------------------------------------
class ImageSelectiveDNNStrategy(ConceptualSafeguardsStrategy):
    """
    True 'Selective DNN' baseline:

    - Uses a separate image DNN model (not the CBM).
    - Loads images from paths using _raw_X_from_split (in our eval helper).
    - Applies a margin-based abstention rule on the image DNN's probabilities.
    - No concept interventions (mask is all False).
    """

    name = DNN_STRATEGY_NAME  # "selective_dnn_cbm" in your CSV
    ...
    def __init__(
        ...
        super().__init__()
        self._dnn = image_dnn.to(device)
        self._device = device
        self._transform = transform if transform is not None else default_image_transform()

    # NOTE: we no longer rely on propose() via ConceptInterventionRunner for this
    # strategy; we evaluate the image DNN directly in eval_image_dnn_on_split.
    # Keeping propose() unused would be confusing, so we omit it entirely here.


# -------------------------------------------------------------------------
# No-intervention conceptual safeguards baseline
# -------------------------------------------------------------------------
class NoInterventionConceptualSafeguardsStrategy(ConceptualSafeguardsStrategy):
    """
    Conceptual Safeguards strategy with interventions disabled:

    - Calls the full ConceptualSafeguardsStrategy.propose(...) to get
      abstention behavior and metrics.
    - Then zeroes out the intervention mask and work metrics.
    - Forces 'after' metrics to equal the 'before' metrics.
    """

    name = NOINT_STRATEGY_NAME
    ...
    def propose(self, model, batch, config: InterventionConfig):
        ...
        sel_before = details.get("selective_acc_before", None)
        cov_before = details.get("coverage_before", None)

        if sel_before is not None:
            details["selective_acc_after"] = float(sel_before)
        if cov_before is not None:
            details["coverage_after"] = float(cov_before)

        # Zero work metrics (no interventions)
        for key in [
        ...
        # Completely disable interventions: zero mask
...
def _inject_work_metrics_from_result(result, metrics: dict) -> None:
    """
    Pulls metrics from the proposal.details into the strat_metrics dict.

    Importantly, this now *also* overwrites:
    - selective_acc_before / selective_acc_after
    - coverage_before / coverage_after

    so that those reflect the strategy's own notion of coverage/accuracy,
    not the runner's generic defaults.
    """
    ...
    # First: overwrite coverage / selective accuracy from details if present
    for key in [
    ...
    # Then: copy all the work-related metrics
    for key in [
        "concepts_checked_per_board",
        "cells_checked_per_board",
        "concepts_checked_per_abstained_board",
...
# -------------------------------------------------------------------------
# Direct eval helper for image DNN baseline
# -------------------------------------------------------------------------
def eval_image_dnn_on_split(
    image_dnn: nn.Module,
    transform: T.Compose,
    split,
    tau: float,
    ...
    """
    Evaluate the image DNN with a margin-based abstention rule on a dataset split.

    Returns a metrics dict with:
      - selective_acc_before / after
      - coverage_before / after
      - work_* metrics (all 0)
    """
    image_dnn.eval()

    paths = list(_raw_X_from_split(split))
    y_true = getattr(split, "y", None)
    if y_true is None:
        raise ValueError("[eval_image_dnn_on_split] split has no 'y' labels.")
    ...
    def _iter_batches():
        ...
        for i in range(0, len(idx), batch_size):
            j = idx[i : i + batch_size]
            batch_paths = [paths[k] for k in j]
            imgs = []
            for p in batch_paths:
                img = Image.open(p).convert("RGB")
                imgs.append(transform(img))
    ...
    with torch.no_grad():
        for j, X_batch in _iter_batches():
            logits = image_dnn(X_batch)  # (B, num_classes)
    ...
    if y_prob.shape[1] == 2:
        ...
        abstain_mask = (p_max >= tau) & (p_max <= 1.0 - tau)
    ...
    coverage = 1.0 - float(abstain_mask.mean())

    metrics = {
        "selective_acc_before": selective_acc,
        "selective_acc_after": selective_acc,
        "coverage_before": coverage,
        "coverage_after": coverage,
        "work_total_concepts": 0.0,
        "work_total_concepts_on_abstained": 0.0,
...
def calibrate_tau_on_validation(
    *,
    ...
    """
    Calibrate tau on the validation split.

    IMPORTANT:
    - For ImageSelectiveDNNStrategy: we bypass ConceptInterventionRunner and
      directly evaluate the image DNN on the val split.
    - For other strategies: we use runner.run as before.
    """
    val_split = None
    ...
    is_image_dnn = isinstance(strategy, ImageSelectiveDNNStrategy)

    for tau in tau_candidates:
        if is_image_dnn:
            metrics = eval_image_dnn_on_split(
                image_dnn=strategy._dnn,
                transform=strategy._transform,
                split=val_split,
                tau=float(tau),
            )
            sel_after = metrics.get("selective_acc_after", None)
        else:
...
def _estimate_total_configs(
    *,
    ...
    For each (label, mechanism, level):
      - DNN runs for each tau
      - NOINT runs for each tau
      - STANDARD runs once
    """
    missing_counts = 0
    ...
    per_label = missing_counts * (2 * len(tau_values) + 1)
    return per_label * len(target_labels)


# -------------------------------------------------------------------------
# CLI + main loop (DNN + standard + no-intervention; all under missingness)
# -------------------------------------------------------------------------
def _parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=results_dir
        / "big_demo"
        / "conceptual_safeguards_metrics_dnn_noint_new_dnn.csv",
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Number of epochs to train the image DNN baseline.",
    )
    parser.add_argument(
        "--image-dnn-batch-size",
        type=int,
        default=64,
        help="Batch size for image DNN training.",
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
        ...
        for target_label, target_value in target_pairs:
            ...
            for mechanism in args.missing_mechanisms:
                ...
                for concept_missing in levels:
                    # Fresh dataset for this (mechanism, missing) combo
                    ...
                    if args.verbose:
                        print(
                            f"[main] Loading dataset from: {args.dataset_file} "
                            f"for mech={mechanism}, missing={concept_missing}"
                        )
                    ...
                    # Settings for metadata (and for things like max_corrupt)
                    ...
                    # --------- Train CBM (on clean concepts; trainer disables missingness internally) ---------
                    ...
                    # Train the image DNN baseline on this dataset (before missingness,
                    # since missingness only affects concepts, not images/labels).
                    image_dnn, img_transform = train_image_dnn_from_dataset(
                        dataset=dataset,
                        epochs=args.image_dnn_epochs,
                        batch_size=args.image_dnn_batch_size,
                        verbose=args.verbose,
                    )
                    dnn_strategy = ImageSelectiveDNNStrategy(
                        image_dnn=image_dnn, device=device, transform=img_transform
                    )

                    # Apply missingness IN PLACE on dataset base concepts
                    ...
                    # ---- τ calibration for each strategy under this missingness ----
                    ...
                    if args.verbose:
                        print(
                            f"[main] Tau (dnn={dnn_tau_values}, "
                            f"noint={noint_tau_values}) "
                            f"for mechanism={mechanism}, missing={concept_missing}"
                        )

                    # ----------------- Selective DNN baseline -----------------
                    dnn_before = len(records)
                    for tau in dnn_tau_values:
                        # For image DNN we do NOT use runner.run; evaluate directly.
                        for split_name, split_data in iter_splits(dataset):
                            if split_name not in INTERVENTION_SPLITS:
                                continue

                            m = eval_image_dnn_on_split(
                                image_dnn=dnn_strategy._dnn,
                                transform=dnn_strategy._transform,
                                split=split_data,
                                tau=float(tau),
                            )

                            # Explicitly ensure work metrics are present and zero
                    ...
                    for tau in noint_tau_values:
                        ...
                        for split_name, split_data in iter_splits(dataset):
                            ...
                            # Enforce 0 work for no-intervention baseline
                            m.setdefault("concepts_checked_per_board", 0.0)
                            m.setdefault("cells_checked_per_board", 0.0)
                            m.setdefault("concepts_checked_per_abstained_board", 0.0)
                            m.setdefault("cells_checked_per_abstained_board", 0.0)
                    ...
                    if args.verbose:
                        ...
                        print(
                            f"[main] No-intervention CS records added for "
                            f"target_label={target_label}, "
                            f"target_value={target_value}, "
                            f"mech={mechanism}, missing={concept_missing}: {noint_added}"
                        )

                    # ----------------- Standard CBM DNN baseline (no abstention) -----------------
                    ...
                    if args.verbose:
                        ...
                        print(
                            ...
                            f"target_label={target_label}, "
                            f"target_value={target_value}, "
                            f"mech={mechanism}, missing={concept_missing}: {standard_added}"
                        )
    ...
    print(
        f"[main] Wrote raw metrics (DNN + standard + no-intervention CS) to {raw_output}"
    )
