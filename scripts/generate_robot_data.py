...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Generate a robot classification dataset."
    )
    parser.add_argument(
        "--seed", type=int, default=1014, help="Random seed (default: 1014)"
    )
    parser.add_argument(
        "--concept-preset",
        choices=["ground_truth", "foot_subtypes"],
        default="ground_truth",
        help="Concept preset: ground_truth (7 ideal) or foot_subtypes (12 subconcepts)",
    )
    parser.add_argument(
        "--no-draw", action="store_true", help="Skip rendering robot PNGs (faster)"
    )
    parser.add_argument(
        "--output",
        type=str,
